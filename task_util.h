...
struct ProjectedBinaryClassificationTaskCreator {
  static void Create(EvalType eval_type,
    ...
    if (task_spec.has_positive_class() &&
      ...
      // There are 1026 tasks (stocks) in total
      for (IntegerT i = 0; i < 1026; i++) {
        std::pair<IntegerT, IntegerT> class_pair(i*10+1, i*10);
      ...
      // I change it to stop random find task. Only from ith task [0, num_tasks]
      // IntegerT num_batch = 2; // the batch here is to get the number of rank_features
      std::pair<IntegerT, IntegerT> selected_pair;
      selected_pair = search_pairs[ith_task];         
