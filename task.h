...
class TaskBuffer {
 public:
  ...
  // assigned correctly.
  std::vector<Matrix<F>> train_features_;
  std::vector<Matrix<F>> valid_features_;
  Scalar industry_relation_;
  EvalType eval_type_;
...
class TaskInterface {
 public:
  ...
  virtual IntegerT industry_relation() const = 0;
 ...
 public:
  explicit Task(const size_t index, const EvalType eval_type,
      ...
      : index_(index),
        ...
        industry_relation_((buffer->industry_relation_)),
  ...
  Task(Task&& other)
      : index_(other.index_),
        ...
        industry_relation_((other.industry_relation_)),
  ...
  Task& operator=(Task&& other) {
    ...
    this->industry_relation_ = (other.industry_relation_),
  ...
  IntegerT industry_relation() const override {return industry_relation_;} // override mean override the above virtual 0?
  ...
  const Scalar industry_relation_;
 private:
...
class TaskIterator {
 public:
  TaskIterator(const std::vector<Matrix<F>>* features,
  ...
  inline const Matrix<F>& GetFeatures() const {
 ...
 private:
  const std::vector<Matrix<F>>* features_;
