...
Instruction::Instruction(
    const Op op, const AddressT in1, const AddressT in2, const AddressT in3, const AddressT in4, const AddressT out)
    : op_(op),
      in1_(in1),
      in2_(in2),
      in3_(in3),
...
Instruction::Instruction(
    const Op op, const AddressT in1, const AddressT in2, const AddressT out, const FloatDataSetter& float_data_setter_0)
    : op_(op),
      in1_(in1),
      in2_(in2),
      out_(out),
...
void Instruction::SetOpAndRandomizeParams(
  ...
  switch (op_) {
    ...
    case MATRIX_GET_COLUMN_OP:
    case MATRIX_GET_ROW_OP:
    case MATRIX_GET_SCALAR_OP:
    ...
    case MATRIX_PREVIOUS_RANK_OP:
    case MATRIX_RELATION_RANK_OP:
    case RELATION_DEMEAN_OP:
    case TS_RANK_OP:
    case TS_ROW_RANK_OP:
    ...
    case CONDITION_OP:
      RandomizeIn1(rand_gen);
      RandomizeIn2(rand_gen);
      RandomizeIn3(rand_gen);
      RandomizeIn4(rand_gen);
      RandomizeOut(rand_gen);    
      return;
    case CORRELATION_OP:
    case COVARIANCE_OP:
...
void Instruction::RandomizeIn3(RandomGenerator* rand_gen) {
  switch (op_) {
    case NO_OP:
    case SCALAR_ABS_OP:
    case SCALAR_HEAVYSIDE_OP:
    ...
    case COVARIANCE_OP:  
      LOG(FATAL) << "Invalid op: " << static_cast<IntegerT>(op_) << std::endl;
    case CONDITION_OP:
      in3_ = rand_gen->ScalarInAddress();
      return;
    // Do not add default clause. All ops should be supported here.
  }
}

void Instruction::RandomizeIn4(RandomGenerator* rand_gen) {
  switch (op_) {
    case NO_OP:
    case SCALAR_ABS_OP:
    case SCALAR_HEAVYSIDE_OP:
...
std::string Instruction::ToString() const {
  ...
  switch (op_) {
    ...
    case SCALAR_DIVISION_OP:
      ...
      break;
    case CONDITION_OP:
      stream << "  s" << out_ << " = s" << in1_ << " > s" << in2_ << " ? s" << in3_ << " : s" << in4_
             << std::endl;
      break; 
    case SCALAR_MIN_OP:
    ...
    case VECTOR_SUM_OP:
      ...
      break;
    case CORRELATION_OP:
      stream << "  s" << out_ << " = correlation(v" << in1_ << ", v" << in2_ 
            << ", " << float_data_0_ << ")" << std::endl;
      break;
    case COVARIANCE_OP:
      stream << "  s" << out_ << " = covariance(v" << in1_ << ", v" << in2_ 
            << ", " << float_data_0_ << ")" << std::endl;
      break;
    case VECTOR_DIFF_OP:
      stream << "  v" << out_ << " = v" << in1_ << " - v" << in2_
    ...
    case MATRIX_ROW_ST_DEV_OP:
      ...
      break;
    case TS_RANK_OP:
      stream << "  s" << out_ << " = TS_rank(s" << in1_ << ")"
             << std::endl;
      break;
    case TS_ROW_RANK_OP:
      stream << "  v" << out_ << " = TS_row_rank(s" << in1_ << ")"
             << std::endl;
      break;
    case MATRIX_PREVIOUS_RANK_OP:
      stream << "  s" << out_ << " = previous_rank(s" << in1_ << ")"
             << std::endl;
      break;
    case SCALAR_GAUSSIAN_SET_OP: {
    ...
    case MATRIX_RELATION_RANK_OP: {
      stream << "  s" << out_ << " = relation_rank(s" << in1_ << ")"
             << std::endl;
      break;
    }
    case RELATION_DEMEAN_OP: {
      stream << "  s" << out_ << " = relation_demean(s" << in1_ << ")"
             << std::endl;
      break;
    }
    case MATRIX_GET_SCALAR_OP: {
      stream << "  s" << out_ << " = get_scalar(" << float_data_0_
             << ", " << float_data_1_ << ")"
             << std::endl;
      break;
    }
    case MATRIX_GET_COLUMN_OP: {
      stream << "  v" << out_ << " = get_column(" << float_data_0_
             << ")"
             << std::endl;
      break;
    }
    case MATRIX_GET_ROW_OP: {
      stream << "  v" << out_ << " = get_row(" << float_data_0_
             << ")"
             << std::endl;
      break;
    }
