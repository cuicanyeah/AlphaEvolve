...
std::pair<std::pair<double, vector<double>>, bool> FECCache::Find(const size_t hash) {
  CachedEvaluation* cached = cache_.MutableLookup(hash);
  double dummy;
  std::vector<double> dummy2;
  if (cached == nullptr) {
    return make_pair(make_pair(dummy, dummy2), false);
  } else {
    const std::pair<double, vector<double>> fitness = cached->fitness;
    ++cached->count;
...
void FECCache::InsertOrDie(
    const size_t hash, const std::pair<double, vector<double>> fitness) {
