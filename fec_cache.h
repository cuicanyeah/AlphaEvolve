...
struct CachedEvaluation {
  std::pair<double, vector<double>> fitness;
  IntegerT count;
  explicit CachedEvaluation(const std::pair<double, vector<double>> fitness) {
...
class FECCache {
 public:
  ...
  std::pair<std::pair<double, vector<double>>, bool> Find(size_t hash);

  // Inserts a hash into the cache, together with its associated fitness. Call
  // only if the hash was not found.
  void InsertOrDie(size_t hash, std::pair<double, vector<double>> fitness);

  // Notes that a hash in the cache has been seen again. Call only if the hash
  // was found.
  void UpdateOrDie(size_t hash, std::pair<double, vector<double>> fitness) {}
